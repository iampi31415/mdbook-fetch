...
/// Fetch remote markdown files.
///
/// This is a unit (fieldless) struct implementing the `Preprocessor`
/// trait (see the "Trait Implementations" section.)
pub struct Fetch;
...
impl Preprocessor for Fetch {
    ...
    /// Modify chapters replacing fragments like `{{#fetch URLs}}` with
    /// the Markdown content the URL points to.
    fn run(
        &self,
    ...
    /// Preprocess Markdown, regardless of the final output being HTML or
    /// Markdown.
    fn supports_renderer(&self, renderer: &str) -> Result<bool> {
        // `bool` was changed here https://github.com/rust-lang/mdBook/commit/4d9095b603bc32f9d590670bfa255b2ddeaeedf6
...
fn fetch(url: String) -> String {
    // Error getting the info is unrecoverable.
    // I don't quite like this line though >_<
